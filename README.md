...
- Final predictions are clipped to be non-negative (`np.clip(..., 0, None)`), and any unmatched submission rows are filled with the training target mean.# Beijing Multi-Site Air Quality — Next-Hour PM2.5 Prediction

### This notebook builds a leakage-safe pipeline to predict `PM2_5_next_hour` for the "Inter-Uni Datathon Stream 2 — Beijing Multi-Site Air Quality" Kaggle competition, using LightGBM and XGBoost with a chronological validation split and a weighted blend.
